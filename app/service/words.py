...
from dependencies.auth import require_entitlements
from model.user import Entitlement
...
def search_words(session: SessionDep, substring: str, page: int, limit: int, current_user: User):
	try:
		show_all = not (current_user == None)
		results = words.get_all_words_from_search(session, transform_input_to_regexp(substring), page, limit, show_all)
		return {
			"words": results[0],
...
def update_word(session: SessionDep, word_id: int, word: Word, user_id: int):
	try:
		words.update_word(session, word_id, word, user_id)
